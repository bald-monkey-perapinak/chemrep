...
│   └── src/
│       ├── components/    # Dashboard, Calendar, Lessons, KnowledgeBase, Students
...
│       ├── api/
│       │   ├── routes/    # auth, knowledge, students, lessons, sessions, voice
...
| **Настройки** | Обновление профиля (имя, платформа по умолчанию). Клонирование голоса: загрузка MP3/WAV-файлов → ElevenLabs Voice Clone API → статус клона |

**Стор (`useStore.js`):**
- `initAuth()` — при старте приложения проверяет токен, загружает профиль преподавателя
- `fetchAll()` — параллельно загружает занятия, учеников, дерево KB
- Все мутации асинхронные: `addLesson`, `deleteLesson`, `addStudent`, `addRootFolder`, `addChildNode`, `renameNode`, `deleteNode`, `addFiles`, `deleteFile`

**API-клиент (`utils/api.js`):**
Покрывает все 39 эндпоинтов: auth, lessons, students, knowledge (классы/разделы/темы/файлы/поиск), sessions, voice. Автоматический logout при 401.

---

### Backend — REST API

Всего **39 эндпоинтов** в 6 роутерах. Все защищены JWT (кроме `/auth/register` и `/auth/login`).
...
- [ ] pgvector + эмбеддинги для RAG (замена keyword matching)
- [ ] WebSocket / SSE — статус урока в реальном времени на фронтенде
- [ ] Страница транскрипта и диалога после урока
- [ ] Уведомления преподавателю (push / email) при ошибке бота
- [ ] Реальная загрузка файлов в S3 / MinIO (сейчас только метаданные)
- [ ] Извлечение текста из PDF/DOCX для RAG (text_extracted)
- [ ] Запись аудио урока и сохранение в S3
