...
- [x] LLM-диалог с RAG по базе знаний (Claude API)
- [x] Управление доской Miro (REST API v2, stub-режим)
- [x] Backend REST API: auth (JWT), lessons, students, sessions, voice
- [x] Отправка домашнего задания по email (SMTP)
- [x] Клонирование голоса преподавателя (ElevenLabs Voice Clone API)
- [x] Фронтенд подключён к реальному API (авторизация, занятия, ученики, KB)
